...
<div class="card"><div class="card-body">
  <div class="table-responsive">
    <table class="table align-middle">
      <thead>
        <tr>
          <th>Fecha</th><th>Usuario</th><th>Proyecto</th><th>Tipo</th>
          <th class="text-end">Monto</th><th>Certificado</th><th class="text-end">Acciones</th>
        </tr>
      </thead>
...
async function cargarCatalogos(){
  try{
    ...
    u.forEach(x=>{
      ...
      selU.add(opt.cloneNode(true)); selUF.add(opt);
    });
    ...
    listP.forEach(x=>{
      ...
      selP.add(opt.cloneNode(true)); selPF.add(opt);
    });
    ...
    listT.forEach(x=>{
      ...
      selT.add(opt.cloneNode(true)); selTF.add(opt);
    });
  }catch(e){ toastError('No se pudieron cargar catálogos'); }
}
function resetFiltros(){ fUsuario.value=''; fProyecto.value=''; fTipo.value=''; fDesde.value=''; fHasta.value=''; fSearch.value=''; pagination.current_page=1; cargarInversiones(); }
function cambiarPagina(d){ const n=pagination.current_page+d; if(n<1||n>pagination.last_page) return; cargarInversiones(n); }
...
async function cargarInversiones(page=null){
  ...
  try{
    ...
    if(data.current_page){
      pagination.current_page=data.current_page; pagination.last_page=data.last_page;
      pageInfo.textContent=`Página ${data.current_page} de ${data.last_page} • ${data.total} registros`;
      prevBtn.disabled=(data.current_page<=1); nextBtn.disabled=(data.current_page>=data.last_page);
    }else{ pageInfo.textContent=`${rows.length} registros`; prevBtn.disabled=nextBtn.disabled=true; }
  }catch(e){ toastError('No se pudo cargar inversiones'); }
}

let modal;
function abrirCrear(){
  tituloModal.textContent='Nueva inversión';
  idInv.value=''; Nombre.value=''; Monto.value=''; Fecha.value='';
  FK_ID_Usuario.value=''; FK_ID_Proyecto.value=''; FK_ID_Tipo.value='';
  Descripcion.value=''; CertificadoInversion.value='';
}
function abrirEditar(inv){
  tituloModal.textContent='Editar inversión';
  idInv.value=inv.ID_Inversion;
  Nombre.value=inv.Nombre??''; Monto.value=inv.Monto??''; Fecha.value=inv.Fecha??'';
  ...
  Descripcion.value=inv.Descripcion??''; CertificadoInversion.value='';
  modal.show();
}
...
async function guardar(e){
  ...
  if(!res.ok){
    ...
    return toastError(j.message||'Error al guardar');
  }
  modal.hide(); toastSuccess('Guardado');
  cargarInversiones();
}
async function eliminar(id){
  if(!confirm('¿Eliminar inversión?')) return;
  const r=await fetch(`${API}/inversiones/${id}`,{method:'DELETE'});
  if(!r.ok) return toastError('No se pudo eliminar');
